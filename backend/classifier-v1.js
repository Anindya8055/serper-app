...
function scoreStructuredSignals(signals, matchedSignals, context) {
  ...
  if (signals.hasCart) {
    ...
    } else if ((hasHardCommerceInBody || knownRetail) && isCommerceUrl(url)) {
      addScore(scores, matchedSignals, "E-commerce", 14, "cart confirmed as storefront");
    } else {
  ...
  if (signals.hasSearchAndFilter && (isCommerceUrl(url) || knownRetail) && !isEditorial && !isInstitutional) {
    addScore(scores, matchedSignals, "E-commerce", 5, "product search/filter");
  }
  ...
  if (signals.hasReviews) {
    ...
    } else if (signals.hasPhone || signals.hasAddress) {
      addScore(scores, matchedSignals, "Small business", 2, "reviews on single business page");
    } else {
      addScore(scores, matchedSignals, "E-commerce", 2, "reviews in commerce context");
    }
  }

  if ((signals.hasCart || hasProductSchema || isStrongRetailProductPath(url, domain)) && knownRetail) {
    subtractScore(scores, matchedSignals, "Directory", 8, "suppress directory on strong commerce");
    subtractScore(scores, matchedSignals, "Small business", 8, "suppress small business on strong commerce");
    addScore(scores, matchedSignals, "E-commerce", 8, "strong retail product override");
  }
