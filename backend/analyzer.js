  ...
  if (/mage\/|Magento_|mage\.cookies|require\.config.*Magento/i.test(h)) {
    ...
    if (isHomepage) {
      ...
      try {
        ...
        if (/travel|wander|nomad|backpack|adventure|journey|abroad|recipe|skinny|healthy|foodie|groom|nutrition|nutriti|frugal|budget.*food|food.*budget|freelanc|creative|design|photo|bootcamp|coding|learn|academy|school|university|college|institute|edu\./i.test(d)) return null;
      } catch {}
